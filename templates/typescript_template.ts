...
for (let i = 0; i < num_runs; i++) {
    ...
    const my_time = new Date().getTime() - my_timer;

    const ai_timer = new Date().getTime();
    ai_function(); // Add any required arguments here
    const ai_time = new Date().getTime() - ai_timer;
...
console.log("My average time:", (my_total_time / num_runs).toFixed(10), "milliseconds");
console.log("AI average time:", (ai_total_time / num_runs).toFixed(10), "milliseconds");
console.log("\n");

console.log("My execution times:");
my_times.forEach((my_time, i) => {
    console.log(`Run ${i+1}: ${my_time.toFixed(10)} milliseconds`);
});

console.log("AI execution times:");
ai_times.forEach((ai_time, i) => {
    console.log(`Run ${i+1}: ${ai_time.toFixed(10)} milliseconds`);
});

if (my_total_time > ai_total_time) {
    const time_factor = my_total_time / ai_total_time;
    console.log(`AI is, on average, ${time_factor.toFixed(10)} times faster.`);
} else {
    const time_factor = ai_total_time / my_total_time;
    console.log(`Mine is, on average, ${time_factor.toFixed(10)} times faster.`);
}
